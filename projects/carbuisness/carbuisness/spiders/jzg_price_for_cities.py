...
website='jzg_price_test_img_2019_for_cities4'
...
class CarSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        sql = "select cityname, cityid from jingzhengu_city"
        dbc.execute(sql)
        res = dbc.fetchall()
        dbc.close()
        ...
        collection = db["modellist_for_cities"]

        data = collection.find()
        for model in data:

            collection.update({"_id":model["_id"]},{'$set':{"status":1}})
            min_year = int(model["nameAvg"]) - 1
            ...
            if max_year == 2019:
                max_year = 2018
            mile = 2 * (int(time.strftime('%Y', time.localtime()))-max_year)
            # print(mile)
            ...
            for c in final_city_list:
                formdata = {
                    "sourcetype": "3",
                    "regdate": "%s-%s-1" % (str(max_year), month),
                ...
                # print(url)
                yield scrapy.Request(url=url_sell, meta=dict({"type":"sell"}, **formdata), callback=self.parse_price, priority=self.p - 1)
                yield scrapy.Request(url=url_buy, meta=dict({"type":"buy"}, **formdata), callback=self.parse_price, priority=self.p - 1)
