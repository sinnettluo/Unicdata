...
website ='che168_price_test'
spidername_new = 'che168_price_test_new'
spidername_update = 'che168_price__test_update'
...
class CarSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        step=len(citylist)/self.parts+1
        starti = self.part * step
        if self.part==self.parts-1:
            step = len(citylist) - starti
        #urllist
        for city in citylist[8:9]:
            for model in modellist[335:336]:
                for year in range(1995,2017+1):
                    for month in range(1,12+1):
                        date = str(year)+'/'+str(month)+'/01'
                        for mile in [1000]+range(5000,605000,5000):
