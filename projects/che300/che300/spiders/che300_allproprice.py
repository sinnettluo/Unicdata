# -*- coding: utf-8 -*-
import scrapy
from ..items import che300_allproprice
import time
        ...
        for i in range(len(t)):
            ...
            for j in da:
                ...
                if cityid in self.citylist:
                    ...
                    # print cityname
                    url = "https://dingjia.che300.com/app/EvalResult/allProvPrices?callback=&\
                        brand="+ brandid +"&series="+ familyid +"&model="+ salesdescid +\
                           "&regDate="+ date +"&mile="+str(milage)

                    yield scrapy.Request(url, meta={"datainfo": datainfo}, callback=self.parse_price,dont_filter=True)

    def parse_price(self,response):
        # print response.url
        item = che300_allproprice()
        ...
        if len(json.loads(response.xpath('//p/text()').extract_first().strip('\\(|\\)')))!=0:
            ...
            modelinfo = data['success']['modelInfo']
            body_type = modelinfo['body_type']
            emission = modelinfo['discharge_standard']
            doors = modelinfo['door_number']
            drive = modelinfo['drive_name']
            engine_power = modelinfo['engine_power']
            geartype = modelinfo['gear_type']
            che300id = modelinfo['id']
            factoryname = modelinfo['maker_name']
            market_date = modelinfo['market_date']
            guideprice = modelinfo['price']
            familyname = modelinfo['sname']
            stopyear = modelinfo['stop_make_year']
            makeyear = modelinfo['year']
            highlight_config = modelinfo['highlight_config']

            priceinfo = data['success']['prices']
            for price in priceinfo:
                localprov = price['provName'].encode('utf-8')
                localprice = price['price']
                item['status'] = md5(response.url+localprov).hexdigest()
        yield item