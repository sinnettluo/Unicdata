...
website ='che300_price_daily_update_test'
...
reids_cli =Redis(host=settings["REDIS_SERVER"],port=6379,db=settings["REDIS_DB"])
...
class CarSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # #modellist
        # with open('blm/'+self.dbname+'/che300.json',"rb") as file:
        #     modellist =json.load(file)
        # with open('blm/'+self.dbname+'/modellist.csv', 'rb') as csvfile:
        with open( 'modellist.csv', 'rb') as csvfile:
        ...
        for row in res:
            ...
            url = "https://dingjia.che300.com/demo/evaluate/freePrice"
            ...
            redis_md5 =md5((url+str( data_quchong)).encode("utf-8")).hexdigest()
            valid =reids_cli.sadd(website,redis_md5)
            print valid
            if valid ==0:
                logging.log(msg="this http requesst is repetition",level=logging.INFO)
                print ("&"*50,"*"*50)
                continue
            else:
            # yield  scrapy.FormRequest(method="post", url=url, meta={"datainfo":meta}, formdata=data, headers=self.headers, callback=self.parse, dont_filter=True)
                yield scrapy.FormRequest(method="post", url=url, meta={"datainfo": meta}, formdata=data,
                                     headers=self.headers, callback=self.parse, dont_filter=True)

    def parse(self, response):

        # logging.log(msg=response.text, level=logging.INFO)
        ...
        logging.log(msg='***************11111***********{}*****************************'.format(item),
                          level=logging.INFO)
        print(item ,"*"*50)
        # yield item
