...
website ='che300_price_daily_residue'
...
class CarSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        with open('blm/'+self.dbname+'/che300.json',"rb") as file:
            modellist_use_list =json.load(file)
        ...
        for row in res:
            ...
            for m in modellist:
                if m["salesdescid"] == row[5] and m["salesdescid"] in modellist_use_list:
                    ...
                    data = {
                        ...
                        "params": '{"brand":"%s","city":"%s","max_reg_Year":%s,"mile":"%s","min_reg_year":%s,"model":"%s","prov":"%s","reg_date":"%s","series":"%s","type":"2"}' % (
                        str(row[3]), str(row[2]), str(max_year), str(row[7]), str(min_year), str(row[5]), str(row[1]),
                        str(row[6]), str(row[4])),
