    ...
    def parse_post(self, response):
        ...
        for anchor in anchors:
            if anchor.xpath("text()").extract_first() == "下一页":
                yield scrapy.Request(url=response.urljoin(anchor.xpath("@href").extract_first()), meta=response.meta, callback=self.parse_post)

        posts = response.xpath("//*[contains(@class, 'l_post j_l_post l_post_bright')]")
        # posts = response.xpath("//*[@class='l_post j_l_post l_post_bright  ']")
        for post in posts:
            item = TiebaItem()
            item['url'] = response.url
            item['grabtime'] = time.strftime('%Y-%m-%d %X', time.localtime())
            item["content"] = post.xpath(".//*[@class='d_post_content j_d_post_content  clearfix']/text()").extract_first().strip()
            ...
            item["post_date"] = data["content"]["date"]
