...
"""
import scrapy
from koubei.items import HangzhouEChong
import time
...
class CarSpider(scrapy.Spider):
    ...
    def parse_details(self, response):
        ...
        item["imgList"] = json.dumps(details["imgList"]) if "imgList" in details else "-"
        ...
        item["status"] = str(details["stationId"]) + "-" + str(item["grabtime"]) + "-" + str(
            details["freeNums"]) + "-" + str(details["acFreeNums"])
