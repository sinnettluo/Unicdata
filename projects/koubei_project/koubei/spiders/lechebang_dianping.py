...
class KoubeiSpider(scrapy.Spider):
    ...
    def parse_product(self, response):
        ...
        for car in cars["result"]:
            ...
            yield scrapy.Request("http://m.lechebang.com/gateway/battery/queryBattery", meta=meta, callback=self.parse_json, dont_filter=True)


    def parse_json(self, response):
        print(response.text)
        for p in json.loads(response.text)["result"]["data"]:
            ...
            # for p in product["itemPriceInfo"]:
                ...
                item['itemname'] = p["battery_model"]
                item['brand'] = p["battery_brand"]
                item['price1'] = p["marketPrice"]
                item['price2'] = p["price"]
                item['cityname'] = response.meta["cityname"]
                item['familyname'] = response.meta["familyName"]
                item['modelname'] = response.meta["modelname"]
                item['status'] = item['cityname'] + item['familyname'] + item['modelname'] + item['itemname'] + item['brand']
